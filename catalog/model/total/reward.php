...
class ModelTotalReward extends Model {
	...
	public function confirm($order_info, $order_total) {
		...
		if ($points) {
			...
			$newdocument=array('customer_reward_id'=>(int)$customer_reward_id, 'customer_id'=>(int)$order_info['customer_id'], 'order_id'=>(int)$order_info['order_id'], 'description'=>sprintf($this->language->get('text_order_id'), (int)$order_info['order_id']), 'points'=>(float)-$points,'date_added'=>new MongoDate(strtotime(date('Y-m-d H:i:s'))));
	...
	public function unconfirm($order_id) {
		...
		$collection="mongo_customer_reward";
		$where=array('order_id'=>(int)$order_id, 'points'=>array('$lt'=>0));
