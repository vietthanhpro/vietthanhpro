...
class ModelTotalVoucher extends Model {
	...
	public function confirm($order_info, $order_total) {
		...
		if ($voucher_info) {
			...
			$voucher_history_id=1+(int)$this->mongodb->getlastid($collection,'voucher_history_id');
			$newdocument=array('voucher_history_id'=>(int)$voucher_history_id, 'voucher_id'=>(int)$voucher_info['voucher_id'], 'order_id'=>(int)$order_info['order_id'], 'amount'=>(float)$order_total['value'],'date_added'=>new MongoDate(strtotime(date('Y-m-d H:i:s'))));
