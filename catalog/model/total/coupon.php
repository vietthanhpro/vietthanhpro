...
class ModelTotalCoupon extends Model {
	...
	public function confirm($order_info, $order_total) {
		...
		if ($coupon_info) {
			...
			$coupon_history_id=1+(int)$this->mongodb->getlastid($collection,'coupon_history_id');
			$newdocument=array('coupon_history_id'=>(int)$coupon_history_id, 'coupon_id'=>(int)$coupon_info['coupon_id'], 'order_id'=>(int)$order_info['order_id'], 'customer_id'=>(int)$order_info['customer_id'], 'amount'=>(float)$order_total['value'],'date_added'=>new MongoDate(strtotime(date('Y-m-d H:i:s'))));
