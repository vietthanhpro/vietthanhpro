...
class ModelToolOnline extends Model {
	public function whosonline($ip, $customer_id, $url, $referer) {
		...
		$where=array('date_added'=>array('$lt'=>new MongoDate(strtotime(date('Y-m-d H:i:s', strtotime('-1 hour'))))));
		...
		$where=array('ip'=>$ip, 'customer_id'=>(int)$customer_id, 'url'=>$data['url'], 'referer'=>$data['referer'], 'date_added'=>new MongoDate(strtotime(date('Y-m-d H:i:s'))));
