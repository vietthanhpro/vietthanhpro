...
class ModelCheckoutOrder extends Model {
	public function addOrder($data) {
		...
		$order_id=1+(int)$this->mongodb->getlastid($collection,'order_id');		
		$newdocument=array('order_id'=>(int)$order_id, 'invoice_no'=>0, 'invoice_prefix'=>$data['invoice_prefix'], 'store_id'=>(int)$data['store_id'], 'store_name'=>$data['store_name'], 'store_url'=>$data['store_url'], 'customer_id'=>(int)$data['customer_id'], 'customer_group_id'=>(int)$data['customer_group_id'], 'firstname'=>$data['firstname'], 'lastname'=>$data['lastname'], 'email'=>$data['email'], 'telephone'=>$data['telephone'], 'fax'=>$data['fax'], 'custom_field'=>isset($data['custom_field']) ? serialize($data['custom_field']) : '', 'payment_firstname'=>$data['payment_firstname'], 'payment_lastname'=>$data['payment_lastname'], 'payment_company'=>$data['payment_company'], 'payment_address_1'=>$data['payment_address_1'], 'payment_address_2'=>$data['payment_address_2'], 'payment_city'=>$data['payment_city'], 'payment_postcode'=>$data['payment_postcode'], 'payment_country'=>$data['payment_country'], 'payment_country_id'=>(int)$data['payment_country_id'], 'payment_zone'=>$data['payment_zone'], 'payment_zone_id'=>(int)$data['payment_zone_id'], 'payment_address_format'=>$data['payment_address_format'], 'payment_custom_field'=>isset($data['payment_custom_field']) ? serialize($data['payment_custom_field']) : '', 'payment_method'=>$data['payment_method'], 'payment_code'=>$data['payment_code'], 'shipping_firstname'=>$data['shipping_firstname'], 'shipping_lastname'=>$data['shipping_lastname'], 'shipping_company'=>$data['shipping_company'], 'shipping_address_1'=>$data['shipping_address_1'], 'shipping_address_2'=>$data['shipping_address_2'], 'shipping_city'=>$data['shipping_city'], 'shipping_postcode'=>$data['shipping_postcode'], 'shipping_country'=>$data['shipping_country'], 'shipping_country_id'=>(int)$data['shipping_country_id'], 'shipping_zone'=>$data['shipping_zone'], 'shipping_zone_id'=>(int)$data['shipping_zone_id'], 'shipping_address_format'=>$data['shipping_address_format'], 'shipping_custom_field'=>isset($data['shipping_custom_field']) ? serialize($data['shipping_custom_field']) : '', 'shipping_method'=>$data['shipping_method'], 'shipping_code'=>$data['shipping_code'], 'comment'=>$data['comment'], 'total'=>(float)$data['total'], 'affiliate_id'=>(int)$data['affiliate_id'], 'commission'=>(float)$data['commission'], 'marketing_id'=>(int)$data['marketing_id'], 'tracking'=>$data['tracking'], 'language_id'=>(int)$data['language_id'], 'currency_id'=>(int)$data['currency_id'], 'currency_code'=>$data['currency_code'], 'currency_value'=>(float)$data['currency_value'], 'ip'=>$data['ip'], 'forwarded_ip'=>$data['forwarded_ip'], 'user_agent'=>$data['user_agent'], 'accept_language'=>$data['accept_language'], 'date_added'=>new MongoDate(strtotime(date('Y-m-d H:i:s'))), 'date_modified'=>new MongoDate(strtotime(date('Y-m-d H:i:s'))));
		...
		if (isset($data['products'])) {
			foreach ($data['products'] as $product) {
				...
				$order_product_id=1+(int)$this->mongodb->getlastid($collection,'order_product_id');
				$newdocument=array('order_product_id'=>(int)$order_product_id, 'order_id'=>(int)$order_id, 'product_id'=>(int)$product['product_id'], 'name'=>$product['name'], 'model'=>$product['model'], 'quantity'=>(int)$product['quantity'], 'price'=>(float)$product['price'], 'total'=>(float)$product['total'], 'tax'=>(float)$product['tax'], 'reward'=>(int)$product['reward']);
				$this->mongodb->create($collection,$newdocument); 
				
				foreach ($product['option'] as $option) {
					//$this->db->query("INSERT INTO " . DB_PREFIX . "order_option SET order_id = '" . (int)$order_id . "', order_product_id = '" . (int)$order_product_id . "', product_option_id = '" . (int)$option['product_option_id'] . "', product_option_value_id = '" . (int)$option['product_option_value_id'] . "', name = '" . $this->db->escape($option['name']) . "', `value` = '" . $this->db->escape($option['value']) . "', `type` = '" . $this->db->escape($option['type']) . "'");
					$collection="mongo_order_option";
					$order_option_id=1+(int)$this->mongodb->getlastid($collection,'order_option_id');
					$newdocument=array('order_option_id'=>(int)$order_option_id, 'order_id'=>(int)$order_id, 'order_product_id'=>(int)$order_product_id, 'product_option_id'=>(int)$option['product_option_id'], 'product_option_value_id'=>(int)$option['product_option_value_id'], 'name'=>$option['name'], 'value'=>$option['value'], 'type'=>$option['type']);
					$this->mongodb->create($collection,$newdocument); 
				}
			}
		}
		...
		if (isset($data['vouchers'])) {
			foreach ($data['vouchers'] as $voucher) {
				...
				$order_voucher_id=1+(int)$this->mongodb->getlastid($collection,'order_voucher_id');
				$newdocument=array('order_voucher_id'=>(int)$order_voucher_id, 'order_id'=>(int)$order_id, 'voucher_id'=>0, 'description'=>$voucher['description'], 'code'=>$voucher['code'], 'from_name'=>$voucher['from_name'], 'from_email'=>$voucher['from_email'], 'to_name'=>$voucher['to_name'], 'to_email'=>$voucher['to_email'], 'voucher_theme_id'=>(int)$voucher['voucher_theme_id'], 'message'=>$voucher['message'], 'amount'=>(float)$voucher['amount']);
				$this->mongodb->create($collection,$newdocument); 
	
				$voucher_id = $this->model_checkout_voucher->addVoucher($order_id, $voucher);	
				//$this->db->query("UPDATE " . DB_PREFIX . "order_voucher SET voucher_id = '" . (int)$voucher_id . "' WHERE order_voucher_id = '" . (int)$order_voucher_id . "'");
				$infoupdate=array('voucher_id'=>(int)$voucher_id);
				$where=array('order_voucher_id'=>(int)$order_voucher_id);
		...
		if (isset($data['totals'])) {
			foreach ($data['totals'] as $total) {
				...
				$order_total_id=1+(int)$this->mongodb->getlastid($collection,'order_total_id');
				$newdocument=array('order_total_id'=>(int)$order_total_id, 'order_id'=>(int)$order_id, 'code'=>$total['code'], 'title'=>$total['title'], 'value'=>(float)$total['value'], 'sort_order'=>(int)$total['sort_order']);
				$this->mongodb->create($collection,$newdocument); 
			}
		}
	...
	public function editOrder($order_id, $data) {
		...
		$collection="mongo_order";
		$infoupdate=array('invoice_prefix'=>$data['invoice_prefix'], 'store_id'=>(int)$data['store_id'], 'store_name'=>$data['store_name'], 'store_url'=>$data['store_url'], 'customer_id'=>(int)$data['customer_id'], 'customer_group_id'=>(int)$data['customer_group_id'], 'firstname'=>$data['firstname'], 'lastname'=>$data['lastname'], 'email'=>$data['email'], 'telephone'=>$data['telephone'], 'fax'=>$data['fax'], 'custom_field'=>isset($data['custom_field']) ? serialize($data['custom_field']) : '', 'payment_firstname'=>$data['payment_firstname'], 'payment_lastname'=>$data['payment_lastname'], 'payment_company'=>$data['payment_company'], 'payment_address_1'=>$data['payment_address_1'], 'payment_address_2'=>$data['payment_address_2'], 'payment_city'=>$data['payment_city'], 'payment_postcode'=>$data['payment_postcode'], 'payment_country'=>$data['payment_country'], 'payment_country_id'=>(int)$data['payment_country_id'], 'payment_zone'=>$data['payment_zone'], 'payment_zone_id'=>(int)$data['payment_zone_id'], 'payment_address_format'=>$data['payment_address_format'], 'payment_custom_field'=>isset($data['payment_custom_field']) ? serialize($data['payment_custom_field']) : '', 'payment_method'=>$data['payment_method'], 'payment_code'=>$data['payment_code'], 'shipping_firstname'=>$data['shipping_firstname'], 'shipping_lastname'=>$data['shipping_lastname'], 'shipping_company'=>$data['shipping_company'], 'shipping_address_1'=>$data['shipping_address_1'], 'shipping_address_2'=>$data['shipping_address_2'], 'shipping_city'=>$data['shipping_city'], 'shipping_postcode'=>$data['shipping_postcode'], 'shipping_country'=>$data['shipping_country'], 'shipping_country_id'=>(int)$data['shipping_country_id'], 'shipping_zone'=>$data['shipping_zone'], 'shipping_zone_id'=>(int)$data['shipping_zone_id'], 'shipping_address_format'=>$data['shipping_address_format'], 'shipping_custom_field'=>isset($data['shipping_custom_field']) ? serialize($data['shipping_custom_field']) : '', 'shipping_method'=>$data['shipping_method'], 'shipping_code'=>$data['shipping_code'], 'comment'=>$data['comment'], 'total'=>(float)$data['total'], 'affiliate_id'=>(int)$data['affiliate_id'], 'commission'=>(float)$data['commission'], 'date_modified'=>new MongoDate(strtotime(date('Y-m-d H:i:s'))));
	...
	public function deleteOrder($order_id) {
		...
		$where=array('order_id'=> (int)$order_id);
		$order=array();
		$order_recurring_list = $this->mongodb->getall($collection,$where, $order);
		$this->mongodb->delete($collection,$where); 
		if ($order_recurring_list) {
			$collection="mongo_order_recurring_transaction";
			foreach ($order_recurring_list as $order_recurring_list_info) {
	...
	public function getOrder($order_id) {
		...
		if ($order_query_data) {			
			...
			$where=array('order_status_id'=>(int)$order_query_data['order_status_id']);
			$order_status_query_data = $this->mongodb->getBy($collection,$where);
			$order_query_data['order_status']=$order_status_query_data['name'];
			//
			...
			return array(
				...
				'order_status'            => $order_query_data['order_status'],
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false) {
		...
		if ($order_info) {
			...
			$collection="mongo_order";
			$infoupdate=array('order_status_id'=>(int)$order_status_id, 'date_modified'=>new MongoDate(strtotime(date('Y-m-d H:i:s'))));
			$where=array('order_id'=>(int)$order_id);
			$this->mongodb->update($collection,$infoupdate,$where);

			//$this->db->query("INSERT INTO " . DB_PREFIX . "order_history SET order_id = '" . (int)$order_id . "', order_status_id = '" . (int)$order_status_id . "', notify = '" . (int)$notify . "', comment = '" . $this->db->escape($comment) . "', date_added = NOW()");
			$collection="mongo_order_history";
			$order_history_id=1+(int)$this->mongodb->getlastid($collection,'order_history_id');
			$newdocument=array('order_history_id'=>(int)$order_history_id, 'order_id'=>(int)$order_id, 'order_status_id'=>(int)$order_status_id, 'notify'=>(int)$notify, 'comment'=>$comment, 'date_added'=>new MongoDate(strtotime(date('Y-m-d H:i:s'))));
			$this->mongodb->create($collection,$newdocument); 
			...
			if (!in_array($order_info['order_status_id'], array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status'))) && in_array($order_status_id, array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status')))) {
				...
				foreach ($order_product_query_data as $order_product) {
					//$this->db->query("UPDATE " . DB_PREFIX . "product SET quantity = (quantity - " . (int)$order_product['quantity'] . ") WHERE product_id = '" . (int)$order_product['product_id'] . "' AND subtract = '1'");
					$collection="mongo_product";
					$where=array('product_id'=>(int)$order_product['product_id'], 'subtract'=>1);
					$info= array('$inc'=> array('quantity'=>-(int)$order_product['quantity']));
					$this->mongodb->incelement($collection,$where, $info);

					//$order_option_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_option WHERE order_id = '" . (int)$order_id . "' AND order_product_id = '" . (int)$order_product['order_product_id'] . "'");
					$collection="mongo_order_option";
					$where=array('order_id'=>(int)$order_id, 'order_product_id'=>(int)$order_product['order_product_id']);
					$order=array();
					$order_option_query_data = $this->mongodb->getall($collection,$where, $order);

					//foreach ($order_option_query->rows as $option) {
					foreach ($order_option_query_data as $option) {
						//$this->db->query("UPDATE " . DB_PREFIX . "product_option_value SET quantity = (quantity - " . (int)$order_product['quantity'] . ") WHERE product_option_value_id = '" . (int)$option['product_option_value_id'] . "' AND subtract = '1'");
						$collection="mongo_product_option_value";
						$where=array('product_option_value_id'=>(int)$option['product_option_value_id'], 'subtract'=>1);
						$info= array('$inc'=> array('quantity'=>-(int)$order_product['quantity']));
						$this->mongodb->incelement($collection,$where, $info);
					}
				}
				...
				$order_total_query_data = $this->mongodb->getall($collection,$where, $order);

				//foreach ($order_total_query->rows as $order_total) {
				foreach ($order_total_query_data as $order_total) {
			...
			if (in_array($order_info['order_status_id'], array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status'))) && !in_array($order_status_id, array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status')))) {
				...
				foreach($product_query_data as $product) {
					//$this->db->query("UPDATE `" . DB_PREFIX . "product` SET quantity = (quantity + " . (int)$product['quantity'] . ") WHERE product_id = '" . (int)$product['product_id'] . "' AND subtract = '1'");
					$collection="mongo_product";
					$where=array('product_id'=>(int)$product['product_id'], 'subtract'=>1);
					$info= array('$inc'=> array('quantity'=>(int)$product['quantity']));
					$this->mongodb->incelement($collection,$where, $info);

					//$option_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_option WHERE order_id = '" . (int)$order_id . "' AND order_product_id = '" . (int)$product['order_product_id'] . "'");
					$collection="mongo_order_option";
					$where=array('order_id'=>(int)$order_id, 'order_product_id'=>(int)$product['order_product_id']);
					$order=array();
					$option_query_data = $this->mongodb->getall($collection,$where, $order);

					//foreach ($option_query->rows as $option) {
					foreach ($option_query_data as $option) {
			...
			if (!$order_info['order_status_id'] && $order_status_id) {
				...
				foreach ($order_product_query_data as $order_product) {
					...
					$product_download_query_data=$product_info['product_download'];

					//if ($product_download_query->row['total']) {
					if (count($product_download_query_data)>0) {
				...
				if ($order_status_query_data) {
					//$order_status = $order_status_query->row['name'];
					$order_status = $order_status_query_data['name'];
				} else {
					$order_status = '';
				}
				...
				$where=array('order_id'=>(int)$order_id);
				$order=array();
				$order_voucher_query_data = $this->mongodb->getall($collection,$where, $order);

				//foreach ($order_voucher_query->rows as $voucher) {
				foreach ($order_voucher_query_data as $voucher) {
					$data['vouchers'][] = array(
				...
				foreach ($order_total_query_data as $total) {
					$data['totals'][] = array(
						'title' => $total['title'],
			...
			if ($order_info['order_status_id'] && $order_status_id && $notify) {
				...
				if ($order_status_query_data) {
					...
					$message .= $order_status_query_data['name'] . "\n\n";
				}
