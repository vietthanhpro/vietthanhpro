...
class ModelLocalisationReturnAction extends Model {
	public function addReturnAction($data) {
		...
		foreach ($data['return_action'] as $language_id => $value) {
			if (isset($return_action_id)) {
				...
				$where=array('return_action_id'=>(int)$return_action_id, 'name'=>$value['name'], 'language_id'=>(int)$language_id);
			} else {
				$return_action_id=1+(int)$this->mongodb->getlastid($collection,'return_action_id');		
				$where=array('return_action_id'=>(int)$return_action_id, 'name'=>$value['name'], 'language_id'=>(int)$language_id);
	...
	public function getReturnAction($return_action_id) {
		...
		$collection="mongo_return_action";
		$where=array('language_id'=>(int)$this->config->get('config_language_id'), 'return_action_id'=>(int)$return_action_id);
	...
	public function getReturnActions($data = array()) {
		...
		if ($data) {/*
			...
			if (!$return_action_data) {/*
				...
				$where=array('language_id' => (int)$this->config->get('config_language_id'));
				$order=array('name'=> 1);
				$return_action_data=(array)$this->mongodb->getall($collection,$where, $order);
	...
	public function getTotalReturnActions() {
		...
		$collection="mongo_return_action";$return_action_data= array();
		$where=array('language_id'=>(int)$this->config->get('config_language_id'));
		$return_action_data=$this->mongodb->gettotal($collection,$where);
		return $return_action_data;
	}
}