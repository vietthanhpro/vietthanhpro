...
class ModelSaleVoucher extends Model {
	public function addVoucher($data) {
		...
		$where=array('voucher_id'=>(int)$voucher_id, 'order_id'=>0, 'code'=>$data['code'], 'from_name'=>$data['from_name'], 'from_email'=>$data['from_email'], 'to_name'=>$data['to_name'], 'to_email'=>$data['to_email'], 'voucher_theme_id'=>(int)$data['voucher_theme_id'], 'message'=>$data['message'], 'amount'=>(float)$data['amount'], 'status'=>(int)$data['status'], 'date_added'=>new MongoDate(strtotime(date('Y-m-d H:i:s'))));
	...
	public function editVoucher($voucher_id, $data) {
		...
		$collection="mongo_voucher";
		$infoupdate=array('voucher_id'=>(int)$voucher_id, 'code'=>$data['code'], 'from_name'=>$data['from_name'], 'from_email'=>$data['from_email'], 'to_name'=>$data['to_name'], 'to_email'=>$data['to_email'], 'voucher_theme_id'=>(int)$data['voucher_theme_id'], 'message'=>$data['message'], 'amount'=>(float)$data['amount'], 'status'=>(int)$data['status'], 'date_added'=>new MongoDate(strtotime(date('Y-m-d H:i:s'))));
	...
	public function getVouchers($data = array()) {
		...
		if ($data) {/*
			...
			$voucher_data = $this->cache->get('voucher');
			if (!$voucher_data) {
				$where=array();
				$order=array('name'=> 1);
				$voucher_theme_data= $this->mongodb->getall($collection,$where, $order);
				$this->cache->set('voucher', $voucher_data);
			}
			return $voucher_data;
		}
	}
	...
	public function getTotalVouchers() {
		...
		$collection="mongo_voucher";
		$where=array();
		$voucher_data=$this->mongodb->gettotal($collection,$where);
		return $voucher_data;
	}
	...
	public function getVoucherHistories($voucher_id, $start = 0, $limit = 10) {
		...
		$voucher_history_query_data = $this->mongodb->getlimit($collection,$where, $order, $start, $limit);
		$this->load->model('sale/order');
		foreach ($voucher_history_query_data as $voucher_history_data_info) {
			$order_info= array();
			$order_info=$this->model_sale_order->getOrder($voucher_history_data_info['order_id']);
			$voucher_history_data[] = array(
				'order_id'=>$voucher_history_data_info['order_id'],
				'customer'=>trim($order_info['firstname'].' '.$order_info['lastname']),
				'amount'=>$voucher_history_data_info['amount'],
				'date_added'=>$voucher_history_data_info['date_added'],
			);
		}
