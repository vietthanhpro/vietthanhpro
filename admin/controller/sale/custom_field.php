...
class ControllerSaleCustomField extends Controller {
	...
	protected function getList() {
		...
		$data_results = $this->model_sale_custom_field->getCustomFields($filter_data);
		$results=$data_results['results'];
		$custom_field_total = $data_results['count'];
		...
		foreach ($results as $result) {
			...
			$data['custom_fields'][] = array(
				...
				'name'            => $result['custom_field_description'][(int)$this->config->get('config_language_id')]['name'],
		...
		$data['sort_name'] = $this->url->link('sale/custom_field', 'token=' . $this->session->data['token'] . '&sort=name' . $url, 'SSL');
		$data['sort_location'] = $this->url->link('sale/custom_field', 'token=' . $this->session->data['token'] . '&sort=location' . $url, 'SSL');
		$data['sort_type'] = $this->url->link('sale/custom_field', 'token=' . $this->session->data['token'] . '&sort=type' . $url, 'SSL');
		$data['sort_status'] = $this->url->link('sale/custom_field', 'token=' . $this->session->data['token'] . '&sort=status' . $url, 'SSL');
		$data['sort_sort_order'] = $this->url->link('sale/custom_field', 'token=' . $this->session->data['token'] . '&sort=sort_order' . $url, 'SSL');
	...
	protected function getForm() {
		...
		//} elseif (isset($this->request->get['custom_field_id'])) {
			...
			$data['custom_field_description'] = $custom_field_info['custom_field_description'];
		} else {
