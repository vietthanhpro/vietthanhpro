...
class ControllerSaleCustomer extends Controller {
	...
	protected function getList() {
		...
		$data_results = $this->model_sale_customer->getCustomers($filter_data);
		$results=$data_results['results'];
		$customer_total = $data_results['count'];
		...
		$data['sort_ip'] = $this->url->link('sale/customer', 'token=' . $this->session->data['token'] . '&sort=ip' . $url, 'SSL');
	...
	protected function validateForm() {
		...
		$data_results = $this->model_sale_custom_field->getCustomFields(array('filter_customer_group_id' => $this->request->post['customer_group_id']));
		$custom_fields=$data_results['results'];
	...
	public function ip() {
		...
		foreach ($results as $result) {
			...
			$data['ips'][] = array(
				...
				'date_added'     => date('d/m/y',$date_added['sec']),
		...
		if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_email'])) {
			...
			foreach ($results as $result) {
				...
				$json[] = array(
					...
					'name'           => strip_tags(html_entity_decode(trim($result['firstname'].' '.$result['lastname']), ENT_QUOTES, 'UTF-8')),
					'customer_group'    => $customer_group,
	...
	public function customfield() {
		...
		$data_results = $this->model_sale_custom_field->getCustomFields(array('filter_customer_group_id' => $customer_group_id));
		$custom_fields=$data_results['results'];
