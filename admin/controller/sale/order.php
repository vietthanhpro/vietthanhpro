...
class ControllerSaleOrder extends Controller {
	...
	protected function getList() {
		...
		$data['sort_order'] = $this->url->link('sale/order', 'token=' . $this->session->data['token'] . '&sort=order_id' . $url, 'SSL');
		...
		$data['sort_total'] = $this->url->link('sale/order', 'token=' . $this->session->data['token'] . '&sort=total' . $url, 'SSL');
		$data['sort_date_added'] = $this->url->link('sale/order', 'token=' . $this->session->data['token'] . '&sort=date_added' . $url, 'SSL');
		$data['sort_date_modified'] = $this->url->link('sale/order', 'token=' . $this->session->data['token'] . '&sort=date_modified' . $url, 'SSL');
	...
	public function getForm() {
		...
		$data_results = $this->model_sale_custom_field->getCustomFields();
		$custom_fields=$data_results['results'];
	...
	public function info() {
		...
		if ($order_info) {
			...
			if ($customer_group_info) {
				$data['customer_group'] = $customer_group_info['customer_group_description'][(int)$this->config->get('config_language_id')]['name'];
			} else {
				$data['customer_group'] = '';
			}
				...
				if ($custom_field['location'] == 'account' && isset($order_info['custom_field'][$custom_field['custom_field_id']])) {
					if ($custom_field['type'] == 'select' || $custom_field['type'] == 'radio') {
						...
						if ($custom_field_value_info) {
							$data['account_custom_fields'][] = array(
								'name'  => $custom_field['custom_field_description'][(int)$this->config->get('config_language_id')]['name'],
								'value' => $custom_field_value_info['custom_field_value_description'][(int)$this->config->get('config_language_id')]['name']
							);
						}
					}
			...
			$order_info_date_added=(array)$order_info['date_added'];
			$order_info_date_modified=(array)$order_info['date_modified'];
			//$data['date_added'] = date($this->language->get('date_format_short'), strtotime($order_info['date_added']));
			//$data['date_modified'] = date($this->language->get('date_format_short'), strtotime($order_info['date_modified']));
			$data['date_added'] = date($this->language->get('date_format_short'), $order_info_date_added['sec']);
			$data['date_modified'] = date($this->language->get('date_format_short'), $order_info_date_modified['sec']);
		...
		foreach ($orders as $order_id) {
			...
			if ($order_info) {
				...
				$data['orders'][] = array(
					...
					'date_added'     => date($this->language->get('date_format_short'),$order_info_date_added['sec']),
