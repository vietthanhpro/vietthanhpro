...
class ControllerSaleVoucher extends Controller {
	...
	protected function getList() {
		...
		$data_results = $this->model_sale_voucher->getVouchers($filter_data);
		$results=$data_results['results'];
		$voucher_total = $data_results['count'];
		...
		foreach ($results as $result) {
			...
			$theme_info = array();
			$theme_info = $this->model_sale_voucher_theme->getVoucherTheme((int)$result['voucher_theme_id']);
			$data['vouchers'][] = array(
				'voucher_id' => $result['voucher_id'],
				'code'       => $result['code'],
				'from'       => $result['from_name'],
				'to'         => $result['to_name'],
				'theme'      => $theme_info['voucher_theme_description'][(int)$this->config->get('config_language_id')]['name'],
		...
		$data['sort_code'] = $this->url->link('sale/voucher', 'token=' . $this->session->data['token'] . '&sort=code' . $url, 'SSL');
		$data['sort_from'] = $this->url->link('sale/voucher', 'token=' . $this->session->data['token'] . '&sort=from_name' . $url, 'SSL');
		$data['sort_to'] = $this->url->link('sale/voucher', 'token=' . $this->session->data['token'] . '&sort=to_name' . $url, 'SSL');
		$data['sort_theme'] = $this->url->link('sale/voucher', 'token=' . $this->session->data['token'] . '&sort=theme' . $url, 'SSL');
		$data['sort_amount'] = $this->url->link('sale/voucher', 'token=' . $this->session->data['token'] . '&sort=amount' . $url, 'SSL');
		$data['sort_status'] = $this->url->link('sale/voucher', 'token=' . $this->session->data['token'] . '&sort=date_end' . $url, 'SSL');
		$data['sort_date_added'] = $this->url->link('sale/voucher', 'token=' . $this->session->data['token'] . '&sort=date_added' . $url, 'SSL');
