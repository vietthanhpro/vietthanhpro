...
class ControllerSaleRecurring extends Controller {
	...
	protected function getList() {
		...
		$data_results = $this->model_sale_recurring->getRecurrings($filter_data);
		$results=$data_results['results'];
		$recurrings_total = $data_results['count'];

		$data['recurrings'] = array();

		foreach ($results as $result) {
			$date_added_=(array)$result['date_added'];
			$date_added = date($this->language->get('date_format_short'),$date_added_['sec']);
		...
		$data['sort_order_recurring'] = $this->url->link('sale/recurring', 'token=' . $this->session->data['token'] . '&sort=order_recurring_id' . $url, 'SSL');
		$data['sort_order'] = $this->url->link('sale/recurring', 'token=' . $this->session->data['token'] . '&sort=order_id' . $url, 'SSL');
		$data['sort_reference'] = $this->url->link('sale/recurring', 'token=' . $this->session->data['token'] . '&sort=reference' . $url, 'SSL');
		$data['sort_customer'] = $this->url->link('sale/recurring', 'token=' . $this->session->data['token'] . '&sort=customer' . $url, 'SSL');
		$data['sort_status'] = $this->url->link('sale/recurring', 'token=' . $this->session->data['token'] . '&sort=status' . $url, 'SSL');
		$data['sort_date_added'] = $this->url->link('sale/recurring', 'token=' . $this->session->data['token'] . '&sort=date_added' . $url, 'SSL');
	...
	public function info() {
		...
		if ($order_recurring) {
			...
			$order_date_added=(array)$result['date_added'];
			//$data['date_added'] = date($this->language->get('date_format_short'), strtotime($order['date_added']));
			$data['date_added'] = date($this->language->get('date_format_short'),$order_date_added['sec']);
