...
class ControllerCatalogProduct extends Controller {
	...
	protected function getList() {
		...
		$data_results = $this->model_catalog_product->getProducts($filter_data);
		$results=$data_results['results'];
		$product_total = $data_results['count'];
		...
		foreach ($results as $result) {
			...
			$data['products'][] = array(
				...
				'image'      => $image,
				'name'       => $result['product_description'][(int)$this->config->get('config_language_id')]['name'],
				'model'      => $result['model'],
				'price'      => $result['price'],
				'special'    => $special,
		...
		$data['sort_name'] = $this->url->link('catalog/product', 'token=' . $this->session->data['token'] . '&sort=name' . $url, 'SSL');
		$data['sort_model'] = $this->url->link('catalog/product', 'token=' . $this->session->data['token'] . '&sort=model' . $url, 'SSL');
		$data['sort_price'] = $this->url->link('catalog/product', 'token=' . $this->session->data['token'] . '&sort=price' . $url, 'SSL');
		$data['sort_quantity'] = $this->url->link('catalog/product', 'token=' . $this->session->data['token'] . '&sort=quantity' . $url, 'SSL');
		$data['sort_status'] = $this->url->link('catalog/product', 'token=' . $this->session->data['token'] . '&sort=status' . $url, 'SSL');
		$data['sort_order'] = $this->url->link('catalog/product', 'token=' . $this->session->data['token'] . '&sort=sort_order' . $url, 'SSL');
		...
		$data['filter_category'] = $filter_category_info['category_description'][(int)$this->config->get('config_language_id')]['name'];
	...
	protected function getForm() {
		...
		//} elseif (isset($this->request->get['product_id'])) {
			...
			$data['product_description'] = $product_info['product_description']; 
		} else {
		...
		if (isset($this->request->post['classname'])) {
			...
			$data['classname'] = $product_info['classname'];
		} else {
			$data['classname'] = '';
		}
		...
		//} elseif (isset($this->request->get['product_id'])) {
			...
			$data['product_store'] = $product_info['product_to_store']; 
		} else {
		...
		if (isset($this->request->post['date_available'])) {
			...
			$date_available=(array)$product_info['date_available'];
			$date_available_value=date('Y-m-d',$date_available['sec']);
			//$data['date_available'] = ($product_info['date_available'] != '0000-00-00') ? $product_info['date_available'] : '';
			$data['date_available'] = ($date_available_value != '0000-00-00') ? $date_available_value : '';
		} else {
		...
		foreach ($categories as $category_id) {
			...
			if ($category_info) {
				$path_info = $this->model_catalog_category->getPath($category_id);
				$data['product_categories'][] = array(
		...
		if (isset($this->request->post['product_tag'])) {
			$tags = $this->request->post['product_tag'];
		} elseif (!empty($product_info)) {
			$tags = $product_info['product_tag'];
		} else {
			$tags = array();
		}
		...
		//} elseif (isset($this->request->get['product_id'])) {
			...
			$filters = $product_info['product_filter'];
		} else {
			$filters = array();
		}
		...
		foreach ($filters as $filter_id) {
			...
			if ($filter_info) {
				$data['product_filters'][] = array(
					...
					'name'      => $filter_info['group'] . ' &gt; ' . $filter_info['filter_description'][$this->config->get('config_language_id')]['name']
				);
			}
		} 
		...
		foreach ($product_attributes as $product_attribute) {
			...
			if ($attribute_info) {
				$data['product_attributes'][] = array(
					...
					'name'                          => $attribute_info['attribute_description'][$this->config->get('config_language_id')]['name'],
		...
		foreach ($product_discounts as $product_discount) {
			$date_start=(array)$product_discount['date_start'];
			$date_end=(array)$product_discount['date_end'];
			$date_start_value=date('Y-m-d',$date_start['sec']);
			$date_end_value=date('Y-m-d',$date_end['sec']);
			$data['product_discounts'][] = array(
				'customer_group_id' => $product_discount['customer_group_id'],
				'quantity'          => $product_discount['quantity'],
				'priority'          => $product_discount['priority'],
				'price'             => $product_discount['price'],
				'date_start'        => ($date_start_value != '0000-00-00') ? $date_start_value : '',
				'date_end'          => ($date_end_value != '0000-00-00') ? $date_end_value :  ''
		...
		foreach ($product_specials as $product_special) {
			$date_start=(array)$product_special['date_start'];
			$date_end=(array)$product_special['date_end'];
			$date_start_value=date('Y-m-d',$date_start['sec']);
			$date_end_value=date('Y-m-d',$date_end['sec']);
			$data['product_specials'][] = array(
		...
		//} elseif (isset($this->request->get['product_id'])) {
			...
			$product_images = $product_info['product_image'];
		} else {
		...
		//} elseif (isset($this->request->get['product_id'])) {
			...
			$product_downloads = $product_info['product_download'];
		} else {
		...
		foreach ($product_downloads as $download_id) {
			...
			if ($download_info) {
				$data['product_downloads'][] = array(
					...
					'name'        => $download_info['download_description'][$this->config->get('config_language_id')]['name']
				);
			}
		}
		...
		foreach ($products as $product_id) {
			...
			if ($related_info) {
				$data['product_relateds'][] = array(
					...
					'name'       => $related_info['product_description'][$this->config->get('config_language_id')]['name']
				);
			}
		}
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_model'])) {
			...
			foreach ($results as $result) {
				...
				foreach ($product_options as $product_option) {
					...
					if ($option_info) {
						...
						foreach ($product_option['product_option_value'] as $product_option_value) {
							...
							if ($option_value_info) {
								$product_option_value_data[] = array(
									'product_option_value_id' => $product_option_value['product_option_value_id'],
									'option_value_id'         => $product_option_value['option_value_id'],
									'name'                    => $option_value_info['option_value_description'][(int)$this->config->get('config_language_id')]['name'],
						...
						$option_data[] = array(
							...
							'product_option_value' => $product_option_value_data,
							'option_id'            => $product_option['option_id'],
							'name'                 => $option_info['option_description'][(int)$this->config->get('config_language_id')]['name'],
				...
				$json[] = array(
					'product_id' => $result['product_id'],
					'name'       => strip_tags(html_entity_decode($result['product_description'][(int)$this->config->get('config_language_id')]['name'], ENT_QUOTES, 'UTF-8')),
