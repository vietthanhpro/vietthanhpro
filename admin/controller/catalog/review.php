...
class ControllerCatalogReview extends Controller {
	...
	protected function getList() {
		...
		$data_results = $this->model_catalog_review->getReviews($filter_data);
		$results=$data_results['results'];
		$review_total = $data_results['count'];
		$this->load->model('catalog/product');
		foreach ($results as $result) {
			$date_added=(array)$result['date_added'];
			$product_info = $this->model_catalog_product->getProduct($result['product_id']);
			if ($product_info) {
				$product_name=$product_info['product_description'][(int)$this->config->get('config_language_id')]['name'];
			} else {$product_name=='';}
			$data['reviews'][] = array(
		...
		$data['sort_author'] = $this->url->link('catalog/review', 'token=' . $this->session->data['token'] . '&sort=author' . $url, 'SSL');
		$data['sort_rating'] = $this->url->link('catalog/review', 'token=' . $this->session->data['token'] . '&sort=rating' . $url, 'SSL');
		$data['sort_status'] = $this->url->link('catalog/review', 'token=' . $this->session->data['token'] . '&sort=status' . $url, 'SSL');
		$data['sort_date_added'] = $this->url->link('catalog/review', 'token=' . $this->session->data['token'] . '&sort=date_added' . $url, 'SSL');
