...
class ControllerCatalogOption extends Controller {
	...
	protected function getList() {
		...
		$data_results = $this->model_catalog_option->getOptions($filter_data);
		$results=$data_results['results'];
		$option_total = $data_results['count'];

		foreach ($results as $result) {
			$data['options'][] = array(
				'option_id'  => $result['option_id'],
				'name'       => $result['option_description'][$this->config->get('config_language_id')]['name'],
	...
	protected function getForm() {
		...
		//} elseif (isset($this->request->get['option_id'])) {
			...
			$data['option_description'] = $option_info['option_description']; 
		} else {
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name'])) {
			...
			$data_results = $this->model_catalog_option->getOptions($filter_data);//data_results
			$options=$data_results['results'];
			...
			foreach ($options as $option) {
				...
				if ($option['type'] == 'select' || $option['type'] == 'radio' || $option['type'] == 'checkbox' || $option['type'] == 'image') {
					...
					foreach ($option_values as $option_value) {
						...
						$option_value_data[] = array(
							'option_value_id' => $option_value['option_value_id'],
							'name'            => strip_tags(html_entity_decode($option_value['option_value_description'][(int)$this->config->get('config_language_id')]['name'], ENT_QUOTES, 'UTF-8')),
							'image'           => $image
						);
					}
				...
				$json[] = array(
					...
					'name'         => strip_tags(html_entity_decode($option['option_description'][(int)$this->config->get('config_language_id')]['name'], ENT_QUOTES, 'UTF-8')),
					'category'     => $type,
					'type'         => $option['type'],
