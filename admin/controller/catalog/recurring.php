...
class ControllerCatalogRecurring extends Controller {
	...
	protected function getList() {
		...
		$data_results = $this->model_catalog_recurring->getRecurrings($filter_data);
		$results=$data_results['results'];
		$recurring_total = $data_results['count'];

		foreach ($results as $result) {
			$data['recurrings'][] = array(
				'recurring_id' => $result['recurring_id'],
				'name'         => $result['recurring_description'][$this->config->get('config_language_id')]['name'],
		...
		$data['sort_name'] = $this->url->link('catalog/recurring', 'token=' . $this->session->data['token'] . '&sort=name' . $url, 'SSL');
		$data['sort_sort_order'] = $this->url->link('catalog/recurring', 'token=' . $this->session->data['token'] . '&sort=sort_order' . $url, 'SSL');
