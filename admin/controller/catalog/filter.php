...
class ControllerCatalogFilter extends Controller {
	...
	protected function getList() {
		...
		$data_results = $this->model_catalog_filter->getFilterGroups($filter_data);
		$results=$data_results['results'];
		$filter_total = $data_results['count'];

		foreach ($results as $result) {
			$data['filters'][] = array(
				'filter_group_id' => $result['filter_group_id'],
				'name'            => $result['filter_group_description'][$this->config->get('config_language_id')]['name'],
		...
		$data['sort_name'] = $this->url->link('catalog/filter', 'token=' . $this->session->data['token'] . '&sort=name' . $url, 'SSL');
		$data['sort_sort_order'] = $this->url->link('catalog/filter', 'token=' . $this->session->data['token'] . '&sort=sort_order' . $url, 'SSL');
	...
	protected function getForm() {
		...
		//} elseif (isset($this->request->get['filter_group_id'])) {
			...
			$data['filter_group_description'] = $filter_group_info['filter_group_description']; 
		} else {
		...
		if (isset($this->request->get['filter_name'])) {
			...
			foreach ($filters as $filter) {
				$json[] = array(
					...
					'name'      => strip_tags(html_entity_decode($filter['group'] . ' &gt; ' . $filter['filter_description'][$this->config->get('config_language_id')]['name'], ENT_QUOTES, 'UTF-8'))
				);
			}
		}
