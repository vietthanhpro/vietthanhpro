...
class ControllerCatalogAttribute extends Controller {
	...
	protected function getList() {
		...
		$data_results = $this->model_catalog_attribute->getAttributes($filter_data);
		$results=$data_results['results'];
		$attribute_total = $data_results['count'];
		...
		foreach ($results as $result) {
			...
			$data['attributes'][] = array(
				...
				'name'            => $result['attribute_description'][$this->config->get('config_language_id')]['name'],
				'attribute_group' => $attribute_group_info['attribute_group_description'][$this->config->get('config_language_id')]['name'],
		...
		$data['sort_name'] = $this->url->link('catalog/attribute', 'token=' . $this->session->data['token'] . '&sort=name' . $url, 'SSL');
		$data['sort_attribute_group'] = $this->url->link('catalog/attribute', 'token=' . $this->session->data['token'] . '&sort=attribute_group_id' . $url, 'SSL');
		$data['sort_sort_order'] = $this->url->link('catalog/attribute', 'token=' . $this->session->data['token'] . '&sort=sort_order' . $url, 'SSL');
	...
	protected function getForm() {
		...
		//} elseif (isset($this->request->get['attribute_id'])) {
			...
			$data['attribute_description'] = $attribute_info['attribute_description']; 
		} else {
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name'])) {
			...
			foreach ($results as $result) {
				...
				$json[] = array(
					'attribute_id'    => $result['attribute_id'],
					'name'            => strip_tags(html_entity_decode($result['attribute_description'][$this->config->get('config_language_id')]['name'], ENT_QUOTES, 'UTF-8')),
					'attribute_group' => $attribute_group_info['attribute_group_description'][$this->config->get('config_language_id')]['name']
					//'attribute_group' => $result['attribute_group']
				);
			}
