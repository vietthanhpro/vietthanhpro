...
class ControllerCatalogDownload extends Controller {
	...
	protected function getList() {
		...
		$data_results = $this->model_catalog_download->getDownloads($filter_data);
		$results=$data_results['results'];
		$download_total = $data_results['count'];
		
		foreach ($results as $result) {
			$date_added=(array)$result['date_added'];//print_r($date_added); die();
			$data['downloads'][] = array(
				'download_id' => $result['download_id'],
				'name'        => $result['download_description'][(int)$this->config->get('config_language_id')]['name'],
		...
		$data['sort_name'] = $this->url->link('catalog/download', 'token=' . $this->session->data['token'] . '&sort=name' . $url, 'SSL');
		$data['sort_date_added'] = $this->url->link('catalog/download', 'token=' . $this->session->data['token'] . '&sort=date_added' . $url, 'SSL');
	...
	protected function getForm() {
		...
		//} elseif (isset($this->request->get['download_id'])) {
			...
			$data['download_description'] = $download_info['download_description']; 
		} else {
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name'])) {
			...
			foreach ($results as $result) {
				$json[] = array(
					'download_id' => $result['download_id'],
					'name'        => strip_tags(html_entity_decode($result['download_description'][$this->config->get('config_language_id')]['name'], ENT_QUOTES, 'UTF-8'))
				);
			}
		}
