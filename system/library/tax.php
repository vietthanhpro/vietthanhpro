...
final class Tax {
	...
	public function setShippingAddress($country_id, $zone_id) {
		...
		$geo_zone_id_data = array();$geo_zone_1_id_list = array();$geo_zone_2_id_list = array();
		$collection="mongo_zone_to_geo_zone";
		$order=array();
		$where=array('country_id'=>(int)$country_id, 'zone_id'=>0); 
		$geo_zone_1_id_list = $this->mongodb->getall($collection,$where, $order);
		foreach ($geo_zone_1_id_list as $geo_zone_1_id_list_info) {
			$geo_zone_id_data[]= (int)$geo_zone_1_id_list_info['geo_zone_id'];
		}
		$where=array('country_id'=>(int)$country_id, 'zone_id'=>(int)$zone_id); 
		$geo_zone_2_id_list = $this->mongodb->getall($collection,$where, $order);
		foreach ($geo_zone_2_id_list as $geo_zone_2_id_list_info) {
			$geo_zone_id_data[]= (int)$geo_zone_2_id_list_info['geo_zone_id'];
		}
		$geo_zone_id_data=array_unique($geo_zone_id_data);
		//
		$tax_rate_id_data = array();$tax_rate_id_list = array();
		$collection="mongo_tax_rate_to_customer_group";
		$where=array('customer_group_id'=>(int)$this->config->get('config_customer_group_id'));
		$order=array();
		$tax_rate_id_list = $this->mongodb->getall($collection,$where, $order);
		foreach ($tax_rate_id_list as $tax_rate_id_list_info) {
			$tax_rate_id_data[]= (int)$tax_rate_id_list_info['tax_rate_id'];
		}
		//
		$tax_query_data = array();
		$collection="mongo_tax_rule";
		$where=array('based'=>'shipping');
		$order=array('priority'=>1);
		$tax_query_data = $this->mongodb->getall($collection,$where, $order);
		foreach ($tax_query_data as $tax_query_data_info) {
			//
			$tax_rate_info = array();
			$collection="mongo_tax_rate";
			$where=array('tax_rate_id'=>(int)$tax_query_data_info['tax_rate_id'], 'tax_rate_id'=>array('$in'=>$tax_rate_id_data), 'geo_zone_id'=>array('$in'=>$geo_zone_id_data));
			$tax_rate_info=$this->mongodb->getBy($collection,$where);
			//
			$this->tax_rates[$tax_query_data_info['tax_class_id']][$tax_rate_info['tax_rate_id']] = array(
				'tax_rate_id' => $tax_rate_info['tax_rate_id'],
				'name'        => $tax_rate_info['name'],
				'rate'        => $tax_rate_info['rate'],
				'type'        => $tax_rate_info['type'],
				'priority'    => $tax_query_data_info['priority']
			);
		}
	}
	...
	public function setStoreAddress($country_id, $zone_id) {
		...
		/*
		$geo_zone_id_data = array();$geo_zone_id_list = array();
		$collection="mongo_geo_zone";
		$where=array();
		$order=array();
		$geo_zone_id_list = $this->mongodb->getall($collection,$where, $order);
		foreach ($geo_zone_id_list as $geo_zone_id_list_info) {
			$geo_zone_id_data[]= (int)$geo_zone_id_list_info['geo_zone_id'];
		}*/		
		$geo_zone_id_data = array();$geo_zone_1_id_list = array();$geo_zone_2_id_list = array();
		$collection="mongo_zone_to_geo_zone";
		$order=array();
